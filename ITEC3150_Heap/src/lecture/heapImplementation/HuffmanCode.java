...
public class HuffmanCode {
  ...
  public static Tree getHuffmanTree(int[] counts) {
    ...
    MinHeap heap = new MinHeap(); // Defined in Listing 24.10
    ...
    while (heap.size() > 1) { 
      Tree t1 = (Tree) heap.remove(); // Remove the smallest weight tree
      Tree t2 = (Tree) heap.remove(); // Remove the next smallest weight 
      heap.add(new Tree(t1, t2)); // Combine two trees
    }

    return (Tree) heap.remove(); // The final tree
  }
