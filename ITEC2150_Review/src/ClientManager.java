 ...
 * 
 * 
 *          This class � This class describes ClientList used to contain the
 *          Pet item ClientList. It also contains the main method which starts
 *          the program.
 * 
...
public class ClientManager
{
	 ...
	 * reading of items from the text file Pet.txt and a menu for allowing user to
	...
	public static void main(String[] args)
	{
		...
		while (!done)
		{
			...
			else if (userInput == 4)
			{
				...
				{
					System.out.println("Is the cat a long-haired cat (true/false)?");
					boolean longHair = keyboard.nextBoolean();
					System.out.println("Is the cat a clawed cat (true/false)?");
					boolean clawed = keyboard.nextBoolean();
					keyboard.nextLine();
					System.out.println("Please enter cat hair color");
					String hairColor = keyboard.nextLine();
					Cat c = new Cat(type, name, ownerName, age, longHair, clawed, hairColor);
					myClientList.addItem(c);
				}
				else if (type.equalsIgnoreCase("dog"))
				{
					...
					String hairColor = keyboard.nextLine();
					Dog d = new Dog(type, name, ownerName, age, breed, hairColor);
					myClientList.addItem(d);
				}
				else
				{
					System.out.println("Does the pet need special handling (true/false)?");
					boolean specialHandling = keyboard.nextBoolean();
					keyboard.nextLine();
					System.out.println("Please enter pet type");
					String petType = keyboard.nextLine();
					Other o = new Other(type, name, ownerName, age, specialHandling, petType);
					myClientList.addItem(o);
				}
			}
			else
			{
